...
curl_setopt_array($curl, array(
  CURLOPT_URL => $flightSeriesBaseUrl.'api/v1/flights/series-check-avail',
...
<form id="flightbookingsubmit" method="post" action="<?php echo $fullurl; ?>flight-booking-action7">
...
<div class="card-header">Departure</span>
...
<div class="col-9">
 ...
 <table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<?php echo date('h:i A',strtotime($layoverFlight->DEP_DATE)); ?> <?php echo $layoverFlight->DEP_TIME; ?></div>
...
(<?php echo strip($res['ORG_CODE']); ?>) <?php echo strip($rscodearr['city']); ?><br /><?php echo strip($rscodearr['airportDescription']); ?></div>
	...
	<?php echo date('h:i A',strtotime($layoverFlight->ARRV_DATE)); ?> <?php echo $layoverFlight->ARRV_TIME; ?></div>
...
(<?php echo strip($res['ORG_CODE']); ?>) <?php echo strip($rscodearr['city']); ?><br />
<?php echo strip($rscodearr['airportDescription']); ?></div>
</div>
	...
	$rs=GetPageRecord('*','flightDestinationMaster',' airportCode="'.$res['DES_CODE'].'"');

$rscodearr=mysqli_fetch_array($rs); ?>

(<?php echo strip($res['DES_CODE']); ?>) <?php echo strip($rscodearr['city']); ?><br /><?php echo strip($rscodearr['airportDescription']); ?></div></td>
...
<button type="button" class="btn btn-danger btn-sm float-left tpbtn" style="position: absolute; right: 80px; background-color:#CC3300;border:1px solid #CC3300; display:none;" onClick="clearfield();" >Clear</button><button type="button" class="btn btn-danger btn-sm float-left tpbtn" style="position: absolute; right: 0px; display:none;" onClick="loadpop('Import Customer',this,'900px')" data-toggle="modal" data-target=".bs-example-modal-center" popaction="action=importcustomertobooking" >Import</button>
...
<?php if($LoginUserDetails['userType']!='client' && $LoginUserDetails['userType']!='b2cwebsite'){  if($totalwalletBalance>=($basefare[1]+$totalfareret)){ ?>
...
<a href="javascript:void(0);" onclick="payonlinenow(<?php echo ($basefare[1]+$totalfareret); ?>)" id="payonlinelink"><button type="button" class="btn btn-danger payonlinebtnmain" id="payonlinebtn">Pay Online ₹<?php echo ($basefare[1]+$totalfareret); ?></button></a>
...
<tr style="display:none;" id="discountAmtDiv">
 ...
 <?php if($LoginUserDetails['userType']!='client' && $LoginUserDetails['userType']!='b2cwebsite'){ ?>
...
changeMonth: true,
 ...
 function payonlinenow(amt){
	$('#flightbooking').val('1');
	$('#bookingMethod').val('0');


childWindow = window.open('online-recharge?b=1&bamount='+amt+'&z=<?php echo encode($_SESSION['agentUserid']); ?>&type=wallet&bType=<?php echo $_SESSION['serviceId']; ?>', '_blank');
