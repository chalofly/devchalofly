...
$result =mysqli_query (db(),"select * from sys_userMaster where email='".$username."' and  password='".($password)."' and status=1 and userType!='agent' and userType!='distributor'")  or die(mysqli_error()); 
$number =mysqli_num_rows($result);  
if($number>0) 
{  
...
$select='cLogin,lLogin,id,currentIp,lastIp,adminyes'; 
...
if($id==1){
$_SESSION['userid']=$id;  
$_SESSION['subadminuserid']=$id;  
} else {


if($userinfo['adminyes']==1){
...
$sql_ins="update sys_userMaster set lLogin='$cLogin',lastIp='$currentIp',cLogin='$clogin',currentIp='$cip',uSession='".$uSession."',firebasetoken='".$ftoken."' where id=".$_SESSION['subadminuserid'].""; 
mysqli_query(db(),$sql_ins) or die(mysqli_error()); 


$sql_insk="insert into userLogs set lLogin='$cLogin',lastIp='$currentIp',cLogin='$clogin',currentIp='$cip',uSession='".$uSession."',firebasetoken='".$ftoken."',userid='".$_SESSION['subadminuserid']."'";
...
function getAttendanceData($userId,$date){
    ...
    $query = GetPageRecord(
        'id,lLogin,checkoutTime',
        'userLogs',
        'userId="'.$userId.'" 
         and DATE(lLogin)="'.$date.'" 
         order by lLogin asc'
    );
    ...
    while($row=mysqli_fetch_array($query)){
        ...
        // FIRST LOGIN
        if($data['firstLogin']==''){
            $data['firstLogin']=$row['lLogin'];
        }

        // LAST UPDATE
        if(
            $row['checkoutTime']!='' &&
            !is_null($row['checkoutTime'])
        ){

            $data['lastUpdate']=$row['checkoutTime'];

        }

        // ONLY CALCULATE IF checkoutTime EXISTS
        if(
            $row['checkoutTime']!='' &&
            !is_null($row['checkoutTime'])
        ){

            $loginTime  = strtotime($row['lLogin']);
            $logoutTime = strtotime($row['checkoutTime']);

            if($logoutTime > $loginTime){

                $sessionMinutes = floor(
                    ($logoutTime - $loginTime) / 60
                );

                $data['totalMinutes'] += $sessionMinutes;
            }
        }
    }
    ...
    // FORMAT HOURS
    $hours   = floor($data['totalMinutes'] / 60);
