...
foreach ($files as $file) {
    if (is_file($file)) {
        ...
        if ($fileDate != $today) {
            unlink($file);
           "Deleted: " . basename($file) . " (Date: $fileDate)<br>";
        } else {
              "Kept: " . basename($file) . " (Date: $fileDate)<br>";
        }
    }
}
...
$rs='';  
$select='*'; 
$where='id="'.$_SESSION['subadminuserid'].'" '; 
...
$rs=GetPageRecord('*','userLogs','userId="'.$_SESSION['subadminuserid'].'" order by id desc'); 
...
send_attachment_mail($fromemail,$LoginParentId['email'],$subject,$mailbody,$ccmail,$attfilename);
