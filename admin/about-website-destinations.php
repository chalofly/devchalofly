...
if($_REQUEST['dltid']!=''){
      deleteRecord('websiteAboutDestination','id="'.decode($_REQUEST['dltid']).'"');  
   ?>
<script>
   alert('Successfully Deleted!');
   window.location.href = 'display.html?ga=about-website-destinations';
</script>
<?php
   } 
?>
...
<a class="dropdown-item neweditpan" onclick="loadpop2('Edit About Destination',this,'600px')" data-toggle="modal" data-target="#myModal2" data-backdrop="static" popaction="action=addwebsiteaboutdestination&id=<?php echo encode($rest['id']); ?>"><i class="fa fa-pencil" aria-hidden="true"></i></a>   
<!-- DELETE BUTTON -->
...
<?php $totalno++; } ?>
  ...
  <script>
   function dltfunction(id)
   {
      ...
      {
        window.location.href = 'display.html?ga=about-website-destinations&dltid='+id;
      }
   }  
 </script> 
