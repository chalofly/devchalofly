...
return $cabin;
 ...
 $imgurl="https://chalofly.com/admin/upload/";
?>
...
<table width="100%" border="1" cellpadding="20" cellspacing="0" bordercolor="#CCCCCC">
  <tr>
	...
	<!--	
			<img src="profilepic/<?php echo stripslashes($agentData['invoiceLogo']); ?>" height="55">
-->			
	 
		<img src="https://chalofly.com/admin/upload/<?php echo $agentData['companyLogo']; ?>" height="55">
    ...
    <td colspan="3"><table width="100%" border="1" cellpadding="10" cellspacing="0" bordercolor="#000000">
      <tr>
		...
		Booking Status: <strong> <?php if($editresult['status']==1 || $editresult['status']==0){ ?>
          ...
          Booking Type: <strong><?php if($editresult['refundyes']=='REFUNDABLE'){ echo 'Refundable'; } else { echo 'Non Refundable'; } ?></strong>  <br />
        ...
        <tr>
            <td colspan="6">
                <div style="padding:5px; ">
                    <?php
                         $i=1;
                    $stprs=GetPageRecord('*','flightBookingStopDetails',' bookingId="'.$editresult['id'].'"'); 
                    while($stopData=mysqli_fetch_array($stprs)){
                   
                    ?>

                        <div style="margin-bottom:8px;">

                            <strong>Stop <?php echo ($i++); ?> : </strong>

                            <?php echo $stopData['stop_name']; ?>

                            (<?php echo $stopData['stop_code']; ?>)

                            &nbsp;&nbsp;|&nbsp;&nbsp;

                            Arrival:
                            <?php echo $stopData['arrival_time']; ?>

                            &nbsp;&nbsp;|&nbsp;&nbsp;

                            Departure:
                            <?php echo $stopData['departure_time']; ?>
...
<?php echo $editresult['flightNo']; ?> </td>
        ...
        <td align="left" valign="top"><strong><?php echo $editresult['flightClass']; ?></strong></td>
      </tr>
      <?php
       if($editresult['flightStop'] > 0)
       { ?>
...
<?php } ?>
	...
	<table width="100%" border="1" cellpadding="4" cellspacing="0" bordercolor="#CCCCCC">
      <tr>
		...
		<?php //echo number_format(($editresult['agentBaseFare']*$countPax)); ?>
        <td colspan="2">Rs.<?php echo number_format(($editresult['agentBaseFare']*$countPax)); ?></td>
        </tr>
      <tr>
        <td width="25%"><strong>Taxes </strong></td>
        <td colspan="2">Rs.<?php echo number_format(($editresult['agentTax']*$countPax)); ?></td>
      </tr>
     <?php if($editresult['seatPrice']>0){ ?> <tr>
      ...
      <tr>
        ...
        <td colspan="2">Rs.<?php echo number_format($editresult['agentTotalFare']*$countPax);  ?></td>
      </tr>
    </table>
