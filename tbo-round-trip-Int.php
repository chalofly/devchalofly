...
foreach($segR as $segmentR)
{
	...
	$inbound[] = $segmentR;		
	   ...
	   $CN_CODE= $airlinecode . ' ' . $fno; 
		...
		$Tax= round($valArray['Fare']['Tax'])+round($valArray['Fare']['OtherCharges'])+round($valArray['Fare']['AdditionalTxnFeeOfrd'])+round($valArray['Fare']['AdditionalTxnFeePub'])+round($valArray['Fare']['PGCharge']);
