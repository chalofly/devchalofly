...
<style>
   ...
   .sortbyaling ul li {background: #fff; float: left; padding: 10px 15px; margin-right: 10px; border: solid 1px var(--bs-link-color); border-radius: 4px; font-size: 14px; color: var(--bs-link-color); font-weight: 700;}
   .sortbyaling ul li a:hover{background: var(--bs-link-color); color: #fff;}
   .sortbyaling ul .active{background: var(--bs-link-color); color: #fff;}
   .itemlist{border: solid 1px #ccc !important;}
   .viewbtnpa{padding: 10px 0 0;}
   .viewbtnpa a{font-size: 16px; font-weight: 600; color: var(--bs-link-color);}
   .bookdetail {
   display: flex;
...
<?php
   ...
   $flightInfos=GetPageRecord('*','wig_flight_json_bkp',' agentId="'.$_SESSION['agentUserid'].'" and uniqueSessionId="'.$_SESSION['uniqueSessionId'].'" group by FLIGHT_NO,FLIGHT_CODE order by AMT asc');
   
   
   
   while($res=mysqli_fetch_array($flightInfos)){

   
   //$b=GetPageRecord('*','wig_flight_json_bkp',' agentId="'.$_SESSION['agentUserid'].'"   and FLIGHT_NO="'.$res['FLIGHT_NO'].'" and DEP_TIME="'.$res['DEP_TIME'].'" and FLIGHT_CODE="'.$res['FLIGHT_CODE'].'" group by FLIGHT_NO,FLIGHT_CODE order by AMT asc');
   
   
   
   $b=GetPageRecord('*','wig_flight_json_bkp',' agentId="'.$_SESSION['agentUserid'].'"   and FLIGHT_NO="'.$res['FLIGHT_NO'].'" and DEP_TIME="'.$res['DEP_TIME'].'" and FLIGHT_CODE="'.$res['FLIGHT_CODE'].'" order by AMT asc');
   ...
   <div class="row" style="padding: 0px 12px;">
      <div class="col-lg-9">
         <div class="bookdetail">
            <div class="bookimg">
               <div class="bkimg"> <img src="<?php echo $imgurl.getflightlogo(stripslashes($res['FLIGHT_NAME'])); ?>" alt=""> </div>
               <h6><?php echo stripslashes($res['FLIGHT_NAME']); ?> <br>
                  <span><?php echo stripslashes($res['FLIGHT_CODE']); ?> <?php echo stripslashes($res['FLIGHT_NO']); ?></span>
               </h6>
            </div>
            ...
            <div class="nonstop">
               ...
               Non Stop
               <?php  }else{ ?>
               <span style="color:#bf0000 !important; cursor:pointer;" id="hoverstop<?php echo $res['id']; ?>" ><?php echo $getstop['STOP'].' Stop '; ?></span>
               <?php } ?>
               </span>
               <div class="stoptooltip" id="stoptooltip<?php echo $res['id']; ?>" ></div>
            </div>
            <div class="bookboxprice">
               <h6><?php echo stripslashes($res['ARRV_TIME']); ?>
                  <?php 
         ...
         <div class="bookmsg">
            <?php if(stripslashes(getfaretypedetails(stripslashes($flightprice['FLIGHT_NAME']),stripslashes($flightprice['PCC'])))!=''){?>
            <p><?php echo stripslashes(getfaretypedetails(stripslashes($flightprice['FLIGHT_NAME']),stripslashes($flightprice['PCC']))); ?></p>
            <?php } ?>
         </div>
               ...
               <tbody>
                  <tr>
                     <td><i class="fa fa-credit-card-alt" aria-hidden="true"></i> <span class="green">
                        <?php if($flightprice['refundyes']=='1'){ echo '<span class="refundablespan">Refundable</span>'; } else { echo '<span class="nonrefundablespan">Non Refundable</span>'; } ?>
                        </span>&nbsp;&nbsp;&nbsp;
                     </td>
                     <td><i class="fa fa-user-circle-o" aria-hidden="true"></i> <span class="red"> <?php echo stripslashes($flightprice['SEAT']); ?> Seat Left </span> </td>
                     <td>
                        <div class="blackbox">
      ...
      <div class="col-lg-3">
         ...
         <a id="booknowlink<?php echo stripslashes($res['id']); ?>" onclick="loadmoreflight('<?php echo stripslashes($res['id']); ?>','<?php echo $res['FLIGHT_NO']; ?>','<?php echo $res['DEP_TIME']; ?>','<?php echo $res['FLIGHT_CODE']; ?>');" style="cursor:pointer;" class="but"><button type="button" class="btn fbtn-danger" style="width:100%;">Select <i class="fa fa-angle-down" aria-hidden="true"></i></button></a>
      </div>
      <!-- 
         <a  id="booknowlink<?php echo stripslashes($res['id']); ?>" onclick="loadmoreflight('<?php echo stripslashes($res['id']); ?>','<?php echo $res['FLIGHT_NO']; ?>','<?php echo $res['DEP_TIME']; ?>','<?php echo $res['FLIGHT_CODE']; ?>');" style="cursor:pointer;" class="btn btn-danger"><button type="button" class="btn btn-danger" style="width:100%;">Select  </button></a>
         
         <a  href="<?php echo $fullurl; ?>flight-review-book?i=<?php echo encode($res['id']); ?>" id="bookNowprice<?php echo stripslashes($res['id']); ?>"><button type="button" class="btn btn-danger" style="width:100%;">Select </button></a>-->  
      <div class="viewbtnpa">
...
<script>
   ...
   $('#pricefa').show();$wrap.find('.item').sort(function(a, b){
   	if(pricefilterid==1){
   		...
   		$('#pricefa').addClass('fa-caret-up');return + a.getAttribute('data-price') - +b.getAttribute('data-price'); 
   	}else{
   		$('#pricefilterid').val('1'); 
   		$('#pricefa').removeClass('fa-caret-up');
   		$('#pricefa').addClass('fa-caret-down');return + b.getAttribute('data-price') -  +a.getAttribute('data-price');
...
<script>
   ...
   $('#moreflight'+id).load('loadmoreflight.php?FLIGHT_NO='+FLIGHT_NO+'&DEP_TIME='+DEP_TIME+'&FLIGHT_CODE='+FLIGHT_CODE+'&RowId='+id);
